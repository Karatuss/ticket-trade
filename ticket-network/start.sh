#!/bin/bash
#
# This script brings up a Hyperledger Fabric network for operaing smart contracts
# and applications.
#
#
# This script is a customized version of the original "fabric-samples/network.sh".
#

ROOTDIR=$(cd "$(dirname "$0")" && pwd)
export PATH=${ROOTDIR}/../bin:${PWD}/../bin:$PATH
export FABRIC_CFG_PATH=${PWD}/configx
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/seller/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for seller and buyer"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.ticket.com docker_peer0.seller.ticket.com docker_peer0.buyer.ticket.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/seller/msp organizations/fabric-ca/seller/tls-cert.pem organizations/fabric-ca/seller/ca-cert.pem organizations/fabric-ca/seller/IssuerPublicKey organizations/fabric-ca/seller/IssuerRevocationPublicKey organizations/fabric-ca/seller/fabric-ca-server.db'
...
CLI_DELAY=3
# channel name defaults to "ticket-channel"
...
CC_COLL_CONFIG="NA"
# channel config policy - @TwoPair
CC_CHANNEL_CONFIG="Channel/Application/Endorsement"
...
CCAAS_DOCKER_RUN=false  # Do not need to ccaas docker build - @TwoPair
# Chaincode version
CC_VERSION="1.0"
...
if [ "$MODE" == "up" ]; then
  ...
  # infoln "I need to rework this... so don't use this command now..."
