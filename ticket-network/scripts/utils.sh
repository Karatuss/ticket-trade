...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    println "Usage: "
    println "  start.sh \033[0;32mup\033[0m [Flags]"
    println
    println "    Flags:"
    ...
    println
    println " Examples:"
    println "   start.sh up createChannel -ca -c mychannel -s couchdb "
    println "   start.sh up createChannel -bft -c mychannel -s couchdb "
  elif [ "$USAGE" == "createChannel" ]; then
    println "Usage: "
    println "  start.sh \033[0;32mcreateChannel\033[0m [Flags]"
    println
    println "    Flags:"
    println "    -bft - Use Orderers with consensus type BFT (Not available in Fabric v2.x)"
    println "    -c <channel name> - Name of channel to create (defaults to \"mychannel\")"
    ...
    println
    println " Examples:"
    println "   start.sh createChannel -c channelName"
    println "   start.sh createChannel -bft"
  elif [ "$USAGE" == "deployCC" ]; then
    println "Usage: "
    println "  start.sh \033[0;32mdeployCC\033[0m [Flags]"
    println
    println "    Flags:"
    ...
    println
    println " Examples:"
    println "   start.sh deployCC -ccn basic -ccp ../asset-transfer-basic/chaincode-javascript/ ./ -ccl javascript"
    println "   start.sh deployCC -ccn mychaincode -ccp ./user/mychaincode -ccv 1 -ccl javascript"
  elif [ "$USAGE" == "deployCCAAS" ]; then
    println "Usage: "
    println "  start.sh \033[0;32mdeployCCAAS\033[0m [Flags]"
    println
    println "    Flags:"
    println "    -c <channel name> - Name of channel to deploy chaincode to"
    println "    -ccn <name> - Chaincode name."
    ...
    println
    println " Examples:"
    println "   start.sh deployCCAAS  -ccn basicj -ccp ../asset-transfer-basic/chaincode-java"
    println "   start.sh deployCCAAS  -ccn basict -ccp ../asset-transfer-basic/chaincode-typescript -ccaasdocker false"  
  else
    println "Usage: "
    println "  start.sh <Mode> [Flags]"
    println "    Modes:"
    ...
    println
    println "    Flags:"
    println "    Used with \033[0;32mstart.sh up\033[0m, \033[0;32mstart.sh createChannel\033[0m:"
    println "    -ca - Use Certificate Authorities to generate network crypto material"
    println "    -bft - Use Orderers with consensus type BFT (Not available in Fabric v2.x)"
    println "    -c <channel name> - Name of channel to create (defaults to \"mychannel\")"
    println "    -s <dbtype> - Peer state database to deploy: goleveldb (default) or couchdb"
    ...
    println "    -verbose - Verbose mode"
    println
    println "    Used with \033[0;32mstart.sh deployCC\033[0m"
    println "    -c <channel name> - Name of channel to deploy chaincode to"
    println "    -ccn <name> - Chaincode name."
