...
func main() {
	...
	result, err = contract.SubmitTransaction("CreateAsset", "ticket13", "yellow", "5", "Tom", "1300")
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
	log.Println(string(result))
	...
	result, err = contract.EvaluateTransaction("ReadAsset", "ticket13")
	if err != nil {
	...
	result, err = contract.EvaluateTransaction("AssetExists", "ticket1")
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v\n", err)
	}
	log.Println(string(result))

	log.Println("--> Submit Transaction: TransferAsset ticket1, transfer to new owner of Tom")
	_, err = contract.SubmitTransaction("TransferAsset", "ticket1", "Tom")
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}

	log.Println("--> Evaluate Transaction: ReadAsset, function returns 'ticket1' attributes")
	result, err = contract.EvaluateTransaction("ReadAsset", "ticket1")
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v", err)
	}
	log.Println(string(result))
